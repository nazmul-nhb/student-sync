...
import type { StudentPayload } from '../types/interfaces';
 ...
 */
export const verifyToken = (secret: string, token?: string): StudentPayload => {
	if (!token) {
		throw new ErrorWithStatus(
			'Authorization Error',
			'Invalid credentials!',
	...
	try {
		return jwt.verify(token, secret) as StudentPayload;
	} catch (_error) {
		throw new ErrorWithStatus(
			'Authorization Error',
